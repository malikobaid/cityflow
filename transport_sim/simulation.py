...
def apply_scenario(graph, scenario, *, city_name=None, tram_lookup=None):
    """
    Apply tramline scenario to graph by adding tram edges between stops.

    Args:
        graph: LightweightGraph instance
        scenario: Scenario configuration dict
        city_name: City name for tram stop lookup
        tram_lookup: Optional pre-loaded tram stop coordinates

    Returns:
        List of node IDs that were added/modified for tramline
    """
    # derive lookup
    ...
    for i in range(len(stops) - 1):
        ...
        n1 = graph.nearest_node(lon1, lat1)
        n2 = graph.nearest_node(lon2, lat2)

        if n1 is not None and n2 is not None:
            # Note: LightweightGraph doesn't support adding edges dynamically
            # In a real implementation, you'd need to modify the graph structure
            # For now, we'll just return the nodes that would be connected
            added.extend([n1, n2])
...
def run_abm(graph, hub, num_agents, agent_distribution, tram_nodes=None, adj_matrix=None):
    """
    ...
    for _ in range(int(num_agents or 0)):
        ...
        # compute shortest-path distance using lightweight graph
        try:
            if isinstance(graph, LightweightGraph):
                # Use the provided traffic-adjusted matrix
                dist = graph.shortest_path_length(home, hub, adj_matrix)
            else:
...
def adjust_for_traffic(G, traffic_level):
    ...
    Args:
        G: Graph object (LightweightGraph or NetworkX)
        traffic_level: Traffic level string

    Returns:
        The same graph (LightweightGraph doesn't support in-place modification)
    """
    level = (traffic_level or "").strip().lower()
    is_peak = level in ("peak", "rush hour", "rush-hour", "rushhour")

    # Check if this is a NetworkX graph (has edges method)
    if hasattr(G, 'edges') and callable(getattr(G, 'edges', None)):
        # NetworkX graph - can modify in place
    ...
    elif isinstance(G, LightweightGraph):
        # LightweightGraph - can't modify in place, but we can set a flag
        # The actual traffic adjustment will be handled in pathfinding
        G._traffic_level = traffic_level
    else:
        # Other graph type - can't modify in place
        pass
